...
### 0. Config Setup (MANDATORY — RUN FIRST, EVERY NEW SESSION)

🛑 **Before any session detection or document work, you MUST ask the user to set up the config.** Never use hardcoded values from `./config.yaml` (it is a blank schema template). Never assume defaults silently.

Ask the user (in a single grouped prompt):

"Before we start, let's set up your session config:

1. **Your name?** (how I'll address you)
2. **Project / context name?**
3. **Communication language?** (the language I'll speak for the whole session)
4. **Output language for the session document?** (press enter to match communication language)
5. **Your skill level?** (beginner / intermediate / expert)
6. **Output folder?** (where I'll save the session document, e.g. `./_bmad-output`)"

**HALT — wait for user answers.**

Then echo back the resolved config and confirm:

"Here's your config — confirm to continue:
- user_name: [...]
- project_name: [...]
- communication_language: [...]
- document_output_language: [... or matches communication]
- user_skill_level: [...]
- output_folder: [...]

Good to proceed? (yes / edit)"

**HALT — only continue once the user confirms.** Hold these values in memory; all later steps reference them. From this point on, speak in `communication_language`.
