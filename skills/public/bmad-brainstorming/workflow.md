...
**ALWAYS ask the user to set up config at the start of every new session.** Do NOT load hardcoded values and do NOT assume any defaults. `./config.yaml` is a schema template only — it documents the keys, not the values.

Before anything else (handled in step-01 Step 0), ask the user for and resolve:

- `user_name`, `project_name`
- `communication_language` (the language used for the whole session)
- `document_output_language` (defaults to `communication_language` if user doesn't specify)
- `user_skill_level` (beginner | intermediate | expert)
- `output_folder` (where session documents are written)
- `date` as system-generated current datetime

Hold these values in memory for the session. Only proceed past setup once the user has confirmed them.
